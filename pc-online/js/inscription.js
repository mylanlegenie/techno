...
email.addEventListener('input', function () {
  ...
  if(email.value.length >= 5){
    ...
    jQuery.ajax({
      url: '/pc-online/inscription/EmailLiveCheck.php', method: "POST",data: myData,
      success: function(reponse){
        var reponse_2 = parseInt(reponse);
        if (reponse==1){
...
pseudonyme.addEventListener('input', function () {
  ...
  if(pseudonyme.value.length > 0){
    var myData = jQuery('#form-inscription').serialize();
    jQuery.ajax({
      url: '/pc-online/inscription/PseudoLiveCheck.php', method: "POST",data: myData,
      success: function(reponse){
